  ...
  <div>
    <popup v-model="showMap" mode="bottom" height="90%" borderRadius="8">
      <div id="bgContainer">
        ...
        <div class="bg-map-box" id="bg-map-box">
          <div class="map-box" :class="{'changeHeight':autoHeight}">
            <div id="map"></div>
            <div class="goMyPosition" @click="goMyPosition"></div>
          </div>
          <div class="showAddress" :class="{'changeHeight':autoHeight}">
            <div class="search-box-adr">
              ...
              <span class="cancel-btn" v-show="focusSearch" @click="cancel">取消</span>
            </div>
            <div class="scoll-view-box" ref="address" v-show="!focusSearch && !locationKeyWord">
              <ul>
                <li
            ...
            <div id="searchResult" class="searchResult" v-show="focusSearch || locationKeyWord">
              <ul v-if="searchResultList.length>0">
                ...
                >
                  ...
                  <div
                    class="addrDetail"
                  >{{item.district}}{{item.address.length==0?"":item.address}}</div>
                </li>
              </ul>
...
export default {
  ...
  methods: {
    ...
    renderMarker() {
      ...
      var icon = new AMap.Icon({
        ...
        image: require("../assets/img/my-position.png"),
    ...
    // 监听高度
    changeHeight() {
      let nearByAddress = this.$refs.address;
      if (nearByAddress.scrollTop >= 10) {
        this.autoHeight = true;
      } else {
        this.autoHeight = false;
      }
  ...
  mounted() {
    ...
    this.$refs.address.addEventListener("scroll", this.changeHeight);
  },
  watch: {
    locationKeyWord() {
...
#bgContainer .head-nav .confirm-btn {
  color: #4c85ff;
}
#bgContainer .search-box-adr {
  width: 90%;
  height: 0.64rem;
  line-height: 0.64rem;
  position: relative;
...
#bgContainer .search-box-adr form {
  width: 80%;
  flex-grow: 1;
  background-color: #f5f8fa;
  border-radius: 0.05rem !important;
}
#bgContainer .search-box-adr > span {
  margin-left: 0.2rem;
}
#bgContainer .search-box-adr form input {
  width: 100%;
  height: 100%;
...
#bgContainer .search-box-adr .search-icon {
  width: 0.26rem;
  height: 0.26rem;
  position: absolute;
  left: 0.15rem;
...
#bgContainer .search-box-adr .cancel-btn {
  font-size: 0.3rem;
  color: #666;
}
...
#bgContainer .bg-map-box .showAddress .scoll-view-box {
  width: 100%;
  height: 100%;
  overflow-y: scroll;
